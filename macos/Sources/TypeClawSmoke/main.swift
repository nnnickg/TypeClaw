...
enum SmokeError: Error, CustomStringConvertible {
    ...
    case wrongLayout(TypeClawLayout)
    ...
    case wrongObservation(TypeClawObservationAction)
...
func typeToken(_ engine: TypeClawEngine, keyCodes: [Int]) throws -> String {
    var hostText = ""
    for keyCode in keyCodes.map(UInt16.init) {
        guard let physical = TypeClawMacKeyCode.physicalKeyIndex(for: keyCode) else {
...
func verifyHostConfigPrecedence() throws {
    ...
    let terminalPolicy = config.resolveInputPolicy(
        facts: TypeClawHostSurfaceFacts(bundleID: "com.googlecode.iterm2")
    )
    ...
    let embeddedTerminalPolicy = config.resolveInputPolicy(
        facts: TypeClawHostSurfaceFacts(
    ...
    let autoDisabledPolicy = config.resolveInputPolicy(
        facts: TypeClawHostSurfaceFacts(bundleID: "com.apple.TextEdit")
    )
...
func verifyAutoDisabledManualLayoutMode() throws {
    let engine = try TypeClawEngine()
    engine.setHostContext(flags: typeclaw_ffi_context_automatic_switching_disabled())
...
do {
    let config = TypeClawEngine.defaultConfig()
