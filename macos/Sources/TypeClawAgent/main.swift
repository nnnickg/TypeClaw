...
private let performanceLogAll = ProcessInfo.processInfo.environment["TYPECLAW_PERF_LOG_ALL"] == "1"
...
private enum TypeClawStartupError: Error, CustomStringConvertible {
    ...
    var description: String {
        ...
        case .inputMonitoringPermissionDenied:
            return "Input Monitoring permission is required. Enable TypeClaw in System Settings > Privacy & Security > Input Monitoring, then relaunch TypeClaw."
        }
    }
}

private func terminateForStartupFailure(_ error: Error) -> Never {
    let message = "TypeClaw startup failed: \(String(describing: error))"
    ...
    alert.messageText = "TypeClaw Failed to Start"
...
private let eventTapCallback: CGEventTapCallBack = { _, type, event, userInfo in
    ...
    let agent = Unmanaged<TypeClawAgent>.fromOpaque(userInfo).takeUnretainedValue()
...
private let inputSourceChangedCallback: CFNotificationCallback = { _, observer, _, _, _ in
    ...
    let agent = Unmanaged<TypeClawAgent>.fromOpaque(observer).takeUnretainedValue()
...
private final class TypeClawAgent: NSObject {
    ...
    init(hostConfig: TypeClawHostConfig, engine: TypeClawEngine) {
    ...
    func start() throws {
        ...
        guard requestInputMonitoringAccessIfNeeded() else {
            throw TypeClawStartupError.inputMonitoringPermissionDenied
        }
        ...
        guard let tap = CGEvent.tapCreate(
            ...
            throw TypeClawStartupError.eventTapCreationFailed
        }

        eventTap = tap
        guard let source = CFMachPortCreateRunLoopSource(kCFAllocatorDefault, tap, 0) else {
            throw TypeClawStartupError.eventTapRunLoopSourceFailed
        }
    ...
    private func processKey(_ event: CGEvent) {
        ...
        do {
            ...
            if let physical = TypeClawMacKeyCode.physicalKeyIndex(for: keyCode) {
    ...
    private struct ManualReplacementToggle {
        ...
        let currentLayout: TypeClawLayout

        func replacementPlan(to targetLayout: TypeClawLayout) -> ReplacementPlan? {
        ...
        private func text(for layout: TypeClawLayout) -> String {
            switch layout {
            case .english:
                return englishText
            case .secondary:
    ...
    private func applyObservation(
        _ action: TypeClawObservationAction,
        engine: TypeClawEngine,
        source: String
    ) {
        let started = ProcessInfo.processInfo.systemUptime
        defer {
    ...
    private func completeInputSourceSelection(
        ...
        if let replacementPlan,
           !replacementFocusIsStillValid(replacementPlan.focus)
        {
    ...
    private func manualToggleReplacementPlan(to targetLayout: TypeClawLayout) -> ReplacementPlan? {
    ...
    private func observeHostBypass(engine: TypeClawEngine, modifiers: UInt8) {
    ...
    private struct ResolvedHostContext {
        ...
        let policy: TypeClawHostInputPolicy
        let facts: TypeClawHostSurfaceFacts
        let pid: pid_t
        let secureInput: Bool
    }
    ...
    private func currentHostContext(engine: TypeClawEngine, reason: String) -> HostContextSnapshot {
    ...
    private func resolveHostContext(
        ...
        if policy.secureInput {
            flags |= typeclaw_ffi_context_secure_input()
        }
        if policy.automaticProcessingDisabled && policy.manualSwitchDisabled {
            flags |= typeclaw_ffi_context_automatic_processing_disabled()
        } else if policy.automaticProcessingDisabled {
            flags |= typeclaw_ffi_context_automatic_switching_disabled()
        }
    ...
    private func configureAccessibilityObserver(
        ...
        basePolicy: TypeClawHostInputPolicy,
        reason: String
    ) {
    ...
    private func shouldRefineWithAccessibility(_ policy: TypeClawHostInputPolicy) -> Bool {
    ...
    private func invalidateHostContext(reason: String) {
        ...
        measured("ffi.setHostContext.invalidate", thresholdMs: slowCallThresholdMs) {
            engine.setHostContext(flags: typeclaw_ffi_context_automatic_processing_disabled())
        }
        engine.resetToken()
    ...
    private func ffiModifiers(from flags: CGEventFlags, keyCode: UInt16? = nil) -> UInt8 {
        ...
        if shiftDown != capsLockAffectsKey {
            modifiers |= UInt8(TC_MOD_SHIFT)
        }
        if flags.contains(.maskControl) {
            modifiers |= UInt8(TC_MOD_CONTROL)
        }
        if flags.contains(.maskAlternate) {
            modifiers |= UInt8(TC_MOD_OPTION)
        }
        if flags.contains(.maskCommand) {
            modifiers |= UInt8(TC_MOD_COMMAND)
        }
        return modifiers
    }
...
private final class InputSourceSwitcher {
    ...
    init(config: TypeClawHostConfig) {
    ...
    func currentLayout() -> TypeClawLayout? {
    ...
    func selectForFuture(_ layout: TypeClawLayout, reason: String) -> Bool {
    ...
    private func inputSourceID(for layout: TypeClawLayout) -> String? {
        switch layout {
        case .english:
    ...
    private static func findInputSourceID(language: String, excluding excludedID: String?) -> String? {
        ...
        return inputSources(includeAllInstalled: true).first { source in
            guard isSelectableKeyboardSource(source),
                  ...
                  sourceID != excludedID,
                  !sourceID.localizedCaseInsensitiveContains("typeclaw")
            else {
                return false
            }
...
do {
    let hostConfig = try TypeClawHostConfig.load()
    let engine = try TypeClawEngine(hostConfig: hostConfig)
    let agent = TypeClawAgent(hostConfig: hostConfig, engine: engine)
    try agent.start()
