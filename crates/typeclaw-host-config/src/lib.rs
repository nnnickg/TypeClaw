...
use typeclaw_core::EngineConfig;
...
impl Config {
    ...
    /// 2. `$TYPECLAW_CONFIG` if set,
    /// 3. `~/.config/typeclaw/config.toml` if it exists,
...
impl HostEnvironment {
    pub fn from_process() -> Self {
        Self {
            config_path: env::var_os("TYPECLAW_CONFIG").map(PathBuf::from),
            data_directory: env::var_os("TYPECLAW_DATA_DIR").map(PathBuf::from),
            pack_directory: env::var_os("TYPECLAW_PACK_DIR").map(PathBuf::from),
...
pub fn home_default_with_environment(environment: &HostEnvironment) -> Option<PathBuf> {
    Some(
        environment
            .home
            .as_ref()?
            .join(".config/typeclaw/config.toml"),
    )
}
...
pub fn default_pack_dir_with_environment(environment: &HostEnvironment) -> Option<PathBuf> {
    ...
    {
        Some(home.join(".local/share/typeclaw/packs"))
    }
}
...
"#,
        ...
        let resolved = ResolvedHostConfig::from_source(
            ConfigSource {
                config,
                path: Some("/tmp/typeclaw.toml".into()),
            },
            &HostEnvironment::default(),
        )
        .unwrap();
        ...
        assert_eq!(resolved.source_path, Some("/tmp/typeclaw.toml".into()));
        assert!(
            resolved
                .app_policy
