...
use typeclaw_core::Layout;
use typeclaw_core::data::{
...
use typeclaw_host_config::{self as host_config, HostEnvironment, ResolvedHostConfig};
...
fn cli() -> Command {
    Command::new("typeclaw")
        ...
        .subcommand(
            Command::new("config")
                .about("Manage TypeClaw CLI configuration")
                .subcommand_required(true)
                .subcommand(
                    Command::new("init")
...
fn cmd_type(args: &[String], explicit_config: Option<&Path>) -> Result<(), String> {
    if args.is_empty() {
        return Err("usage: typeclaw type <KEYS> [<KEYS>...]".into());
    }
...
fn cmd_stream(args: &[String], explicit_config: Option<&Path>) -> Result<(), String> {
    if !args.is_empty() {
        return Err("usage: typeclaw stream  (reads tokens from stdin, one per line)".into());
    }
    let source = load_config(explicit_config)?;
    let mut engine = build_engine(&source.config)?;
    ...
    if stdin.is_terminal() {
        eprintln!("typeclaw stream: reading tokens from stdin (one per line); Ctrl-D to end.");
    }
...
fn cmd_predict(args: &[String], explicit_config: Option<&Path>) -> Result<(), String> {
    ...
    if tokens.is_empty() {
        return Err("usage: typeclaw predict [--json] <KEYS>".into());
    }
...
fn eval_cases(args: &[String], engine: &Engine) -> Result<Vec<EvalCase>, String> {
    match args {
        ...
        _ => Err("usage: typeclaw eval [--generated [limit-per-layout] | <tsv>]".into()),
    }
}
...
fn builtin_eval_cases() -> Vec<EvalCase> {
    [
        ...
        ("en_typeclaw", "typeclaw", Layout::English),
...
fn cmd_model(args: &[String], explicit_config: Option<&Path>) -> Result<(), String> {
    if !args.is_empty() {
        return Err("usage: typeclaw model".into());
    }
...
fn cmd_pack(args: &[String], explicit_config: Option<&Path>) -> Result<(), String> {
    match args.first().map(String::as_str) {
        Some("install") => {
            if args.len() != 2 {
                return Err("usage: typeclaw pack install <PACK_DIR>".into());
            }
            install_pack(Path::new(&args[1]), explicit_config)
        }
        Some("list") => {
            if args.len() != 1 {
                return Err("usage: typeclaw pack list".into());
            }
            list_packs(explicit_config)
        }
        Some("use") => {
            if args.len() != 2 {
                return Err("usage: typeclaw pack use <ID>".into());
            }
            use_pack(args[1].as_str(), explicit_config)
        }
        Some("inspect") => {
            if args.len() != 2 {
                return Err("usage: typeclaw pack inspect <PACK_DIR|ID>".into());
            }
            inspect_pack(args[1].as_str(), explicit_config)
        }
        Some(other) => Err(format!(
            "unknown pack subcommand: {other}\n\nusage: typeclaw pack <install|list|use|inspect>"
        )),
        None => Err("usage: typeclaw pack <install|list|use|inspect>".into()),
    }
}
...
fn pack_dir_or_error(config: &Config) -> Result<PathBuf, String> {
    configured_pack_dir(config).ok_or_else(|| {
        "could not resolve pack directory; set TYPECLAW_PACK_DIR or [packs].directory".to_owned()
    })
}
...
fn writable_config_path(explicit_config: Option<&Path>) -> Result<PathBuf, String> {
    explicit_config
        ...
        .ok_or_else(|| {
            "could not locate ~/.config/typeclaw/config.toml; pass --config <path>".to_owned()
        })
}
    ...
    match args.first().map(String::as_str) {
        Some("init") => {
            ...
            let target = target.ok_or_else(|| {
                "could not locate ~/.config/typeclaw/config.toml; pass an explicit path".to_owned()
            })?;
            if target.exists() {
                return Err(format!(
        ...
        Some(other) => Err(format!(
            "unknown config subcommand: {other}\n\nusage: typeclaw config <init|show> [path]"
        )),
        None => Err("usage: typeclaw config <init|show> [path]".into()),
    }
}

fn cmd_completions(args: &[String]) -> Result<(), String> {
    let [shell] = args else {
        return Err("usage: typeclaw completions <bash|elvish|fish|powershell|zsh>".into());
    };
    let shell = parse_completion_shell(shell)?;
    let mut command = cli();
    generate(shell, &mut command, "typeclaw", &mut io::stdout());
    Ok(())
}
...
fn redraw(
    ...
    let _ = writeln_cr(
        &mut buf,
        "TypeClaw REPL — Esc/Ctrl-C quit | Backspace delete | Space/Enter end-of-token",
    );
