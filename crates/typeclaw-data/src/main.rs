...
use typeclaw_core::{KeyboardMap, PhysicalKey};
...
fn usage() -> &'static str {
    "typeclaw-data — build TypeClaw language artifacts.

Usage:
  typeclaw-data
      rebuild embedded EN/UK artifacts under crates/typeclaw-core/data
  typeclaw-data build-embedded
      same as no arguments
  typeclaw-data build-pack <SPEC.toml> --out <PACK_DIR> [--cache <DIR>] [--force]
...
fn build_embedded_artifacts() -> Result<()> {
    ...
    let cache_dir = workspace_root.join("target").join("typeclaw-data-cache");
...
fn parse_build_pack_args(args: &[String]) -> Result<PackBuildArgs> {
    if args.is_empty() {
        bail!("usage: typeclaw-data build-pack <SPEC.toml> --out <PACK_DIR>");
    }
